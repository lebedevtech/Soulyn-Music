...
def kb_cancel_create(uid):
    kb = InlineKeyboardBuilder()
    kb.button(text=T(uid, 'btn_cancel_create'), callback_data="open:playlists")
    return kb.as_markup()
    ...
    if user and user.get("playlists"):
        for pl_name in user["playlists"]:
            if pl_name == "Favorites":
                ...
                # 👇 Умная проверка: отделяем иконку от имени
                icon, clean_name = split_playlist_name(pl_name)
                if icon:
                    label = f"{icon} {clean_name}" # Если иконка уже есть - используем её
                else:
                    label = f"📂 {clean_name}" # Если нет - добавляем папку
...
def kb_track(uid, vid, from_playlist=None, is_liked=False):
    ...
    if from_playlist == "Favorites":
        ...
        if is_liked:
            kb.button(text="💔", callback_data=f"unfav:{vid}")
        else:
            kb.button(text="❤️", callback_data=f"fav:{vid}")
    ...
    if user and user.get("status") == "user":
        if from_playlist:
            ...
            if from_playlist == "Favorites": kb.adjust(1, 2)
            else: kb.adjust(2, 2) 
        else:
